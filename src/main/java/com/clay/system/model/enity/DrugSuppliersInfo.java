...
public class DrugSuppliersInfo
{
    ...
    //供应商名
    @Column(name = "name",columnDefinition = "varchar(66) COMMENT ''",nullable = false)
    private String name;

    @Size(max = 15,message = "供应商商标不可超过15字")
    @NotBlank(message = "商标不可为空")
    //商标，无则和name相同
    @Column(name = "brand",columnDefinition = "varchar(20) COMMENT '商标'",nullable = false)
    private String brand;

    @NotBlank(message = "联系电话不可为空")
    @Size(min = 5,max = 13,message = "电话请控制在长度5-13以内")
    //电话
    @Column(name = "phoneNum",columnDefinition = "varchar(11) COMMENT '电话'",nullable = false)
    private String phoneNum;

    @Email(message = "请注意email格式")
    @Size(min = 5,max = 40,message = "邮箱长度请控制在5-40内")
    //Email
    @Column(name = "email",columnDefinition = "varchar(50) COMMENT '邮箱'",nullable = false)
    private String email;

    @NotBlank(message = "联系地址不可为空")
    @Size(min = 2,max = 100,message = "联系地址长度请控制在2-100内")
    //地址
    @Column(name = "address",columnDefinition = "varchar(120) COMMENT '地址'",nullable = false)
    private String address;

    //供应商对象hashcode,在其它信息已设定的情况下
    @Column(name = "supplierCode",columnDefinition = "int COMMENT '根据后端hash出来的代码'",nullable = false)
